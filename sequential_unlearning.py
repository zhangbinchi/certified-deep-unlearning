...
def load_sequential_data(dataset, num_unlearn):
    torch.manual_seed(0)
    res_set, unl_set = random_split(dataset, [len(dataset) - num_unlearn, num_unlearn])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num-unlearn', default=100, type=int, help='number of unlearned samples')
    parser.add_argument('--num-sequential', default=10, type=int, help='number of sequential iterations')
    ...
    if not os.path.exists('./model'):
        os.mkdir('./model')
    if not os.path.exists('./model/sequential'):
        os.mkdir('./model/sequential')

    PATH_load = './model/' + args.model + '_' + args.dataset + f"_lr_{str(args.lr).replace('.', '_')}" + f"_bs_{str(args.batch_size)}" + f"_wd_{str(args.weight_decay).replace('.', '_')}" + f"_epoch_{str(args.epochs)}" + f"_seed_{str(args.seed)}" + '.pth'
    PATH_save = './model/sequential/unlearn_' + args.model + '_' + args.dataset + f"_lr_{str(args.lr).replace('.', '_')}" + f"_bs_{str(args.batch_size)}" + f"_wd_{str(args.weight_decay).replace('.', '_')}" + f"_epoch_{str(args.epochs)}" + f"_seed_{str(args.seed)}" + f"_num_{str(args.num_unlearn)}" + f"_unlearnbs_{str(args.unlearn_bs)}" + f"_s1_{str(args.s1)}" + f"_s2_{str(args.s2)}" + f"_std_{str(args.std).replace('.', '_')}" + f"_gamma_{str(args.gamma).replace('.', '_')}" + f"_scale_{str(args.scale).replace('.', '_')}" + '.pth'

    res_set, _ = load_dataset(args.dataset)
    num_classes = 10
    ...
    for i in range(args.num_sequential):
        unl_set, res_set = load_sequential_data(res_set, args.num_unlearn)
        ...
        for j, param in enumerate(model.parameters()):
            param.data.add_(-delta[j])
        state_dict = model.state_dict().copy()
        for param in state_dict:
            if state_dict[param].requires_grad == True:
                state_dict[param].data.add_(args.std * torch.randn(state_dict[param].data.size()).to(args.device))
        torch.save(state_dict, PATH_save+'_'+str(i)+'.pth')
